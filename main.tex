...
Other parameters, however, are unique to multivariate Brownian models, like the character values from all characters at the root of the tree ($\boldsymbol{y_0}$), a vector containing all relative character-specific evolutionary rates ($\boldsymbol{r}$), and the between-character correlation matrix ($\boldsymbol{\rho}$).
...
\section{Bayesian phylogenetic inference using multiple continuous morphological characters (and more!)}
...
\textcolor{orange}{Like in other tutorials, we will use BEAUti2 as a tool for specifying BEAST2 .XML files for running our analyses.}
...
\begin{figure}[!htbp]
    ...
    \caption{Reading fossil ages (heights) as tip sampling dates.}
    \label{fig:example2}
\end{figure}

In the populated table (Fig. \autoref{fig:example3}), columns \textbf{Date} and \textbf{Age/Height} should now have values between 0.0 and 35.55 million years.
...
\subsubsection{Seting up the Brownian motion model}
